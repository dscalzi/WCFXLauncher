...
//		this.bindContextMenu();
	...
	private void bindProgressBar(){
		BooleanBinding overrideVisibility = Bindings.createBooleanBinding(() -> container.isVisible() && engine.getLoadWorker().getState() == State.RUNNING, container.visibleProperty(), engine.getLoadWorker().stateProperty());
...
//	}
	...
	private String getJQueryIfNeeded() {
		Object res;
		try {
			res = engine.executeScript("$");
		} catch (Exception e) {
			res = null;
		}
		...
		if(res == null) {
			try {
				return new String(Files.readAllBytes(Paths.get(getClass().getResource("/scripts/jquery-3.5.1.min.js").toURI())));
			} catch (Throwable t) {
				return "";
			}
		}
	...
	private void setupJavaScript(){
		...
		String jQuery = getJQueryIfNeeded();
		try(InputStream is = getClass().getResourceAsStream("/scripts/web.js")){
			javascript = loadLocalScript(is);
			engine.getLoadWorker().stateProperty().addListener((ov, oldState, newState) -> {
				if (newState == State.SUCCEEDED) {
					engine.executeScript(jQuery + javascript);
				}
			});
		} catch (IOException e) {
...
//		WeakReferenceQueue<Window> wQ = new WeakReferenceQueue<>();
...
//	                        	if(itemsContainer.getChildren().get(i) instanceof ContextMenuContent.MenuItemContainer){
//		                        	ContextMenuContent.MenuItemContainer item = (ContextMenuContent.MenuItemContainer) itemsContainer.getChildren().get(i);
...
//	}
	 ...
	 * {@see com.sun.javafx.scene.control.ContextMenuContent#updateVisualItems()}
	 * 
	 * @return
	 */
